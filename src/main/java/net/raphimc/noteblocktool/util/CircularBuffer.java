...
public class CircularBuffer {
    ...
    public void addAll(final int[] values, final int valuesLength) {
    ...
    public int takeSafe() {
        ...
        final int value = this.buffer[this.head];
        this.head = (head + 1) % this.buffer.length;
        this.currentSize--;
        return value;
    }

    public int[] takeAllSafe(final int size) {
        final int[] values = new int[size];
        this.takeAllSafe(values);
        return values;
    }

    public void takeAllSafe(final int[] values) {
        /*if (size > currentSize) {
        ...
        Arrays.fill(values, elementsToRemove, values.length, 0);
    }
